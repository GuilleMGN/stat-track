...
// Guild member update for queue role changes
client.on('guildMemberUpdate', async (oldMember, newMember) => {
  ...
  for (const queue of queues) {
    ...
    if (!channel) {
      console.error(`Queue channel ${channel_id} not found in guild ${guildId}`);
      continue;
    }

    const msgId = (await getQuery('settings', { key: `queue_message_${channel_id}`, guild_id: guildId }))?.value;
    if (!msgId) {
      console.log(`No queue message ID found for channel ${channel_id} in guild ${guildId}`);
      continue;
    }

    const queueMsg = await channel.messages.fetch(msgId).catch(() => null);
    if (!queueMsg) {
      console.log(`Queue message ${msgId} not found in channel ${channel_id}`);
      // Create a new queue message if it doesn't exist
      ...
      await runQuery('settings', 'UPDATE', { key: `queue_message_${channel_id}`, guild_id: guildId }, { value: newQueueMsg.id })
        .catch(async (error) => {
          if (error.code === 11000) {
            await runQuery('settings', 'INSERT', { key: `queue_message_${channel_id}`, guild_id: guildId }, { value: newQueueMsg.id });
          } else {
            console.error(`Error setting new queue message ID for channel ${channel_id}:`, error);
          }
        });
      continue;
    }
    ...
    if (!embed) {
      console.log(`No embed found in queue message ${msgId} in channel ${channel_id}`);
      continue;
    }
    ...
    if (!hadRole && hasRole) {
      ...
      await queueMsg.edit({ embeds: [embed] }).catch(err => {
        console.error(`Error editing queue message ${msgId} in channel ${channel_id}:`, err);
      });
      ...
      // Stop the queue cycle if no players remain
      if (count === 0) {
        const activeQueue = client.activeQueues.get(channel_id);
        if (activeQueue) {
          if (activeQueue.timer) clearTimeout(activeQueue.timer);
    ...
    if (count === 10) {
      try {
        ...
        embed = EmbedBuilder.from(embed)
          ...
          .setTimestamp();
        await queueMsg.edit({ embeds: [embed] }).catch(err => {
          console.error(`Error editing queue message ${msgId} on match creation failure:`, err);
        });
      }
    }
  }
...
async function updateQueuePeriodically(db, channel, queue, role_id, guildId) {
  try {
    ...
    let oldMsg;
    if (msgId) {
      oldMsg = await channel.messages.fetch(msgId).catch(() => null);
    ...
    // If count reaches 10, trigger match creation
    if (count === 10) {
      await createMatch(db, channel, players, guildId);
    ...
    console.error(`Error in updateQueuePeriodically for channel ${queue.channel_id}:`, error);
  }
}
        ...
        try {
          ...
          try {
            ...
            const result = await collection.updateOne(
              { match_number: matchNumber, guild_id: guildId },
              {
                $setOnInsert: {
                  ct_team: ctTeam.join(','),
                  tr_team: trTeam.join(','),
                  ...
                  mvp1: null,
                  mvp2: null,
                  bonus: parseInt((await getQuery('settings', { key: `queue_bonus_${channelId}`, guild_id: guildId }))?.value || 0)
                }
              },
              { upsert: true }
            );
            ...
            if (result.upsertedCount === 0 && result.matchedCount === 0) {
              // No match was inserted or updated, likely due to a race condition or error
              console.warn(`No match inserted or updated for match_number ${matchNumber} in guild ${guildId}. Incrementing match number.`);
              matchNumber = await getNextMatchNumber(guildId);
            ...
            console.log(`Match #${matchNumber} saved successfully for guild ${guildId}`);
          } catch (error) {
            console.error(`Error saving match #${matchNumber} for guild ${guildId}:`, error);
            throw new Error(`Failed to save match #${matchNumber} to database: ${error.message}`);
          }
          ...
          if (resultsChannelId) {
            ...
            if (resultsChannel) {
              ...
              await resultsChannel.send({ embeds: [resultsEmbed] }).catch(err => {
                console.error(`Failed to send match #${matchNumber} to results channel ${resultsChannelId}:`, err);
              });
            } else {
              console.warn(`Results channel ${resultsChannelId} not found for guild ${guildId}`);
            }
          }
          ...
          await interaction.message.delete().catch(err => {
            console.error(`Failed to delete match message for match #${matchNumber}:`, err);
          });
          const newMatchEmbed = EmbedBuilder.from(matchEmbed).setTitle(`Match #${matchNumber}`);
          await interaction.channel.send({ content: `${rolePing} Match confirmed!`, embeds: [newMatchEmbed], components: [disabledRow] }).catch(err => {
            console.error(`Failed to send confirmed match #${matchNumber} embed:`, err);
          });
          ...
          if (queue) {
            ...
            if (queueChannel) {
              const newQueueEmbed = new EmbedBuilder()
                ...
                .setTimestamp();
              const newQueueMsg = await queueChannel.send({ embeds: [newQueueEmbed] }).catch(err => {
                console.error(`Failed to send new queue embed for channel ${queue.channel_id}:`, err);
                throw err;
              });
              await runQuery('settings', 'UPDATE', { key: `queue_message_${queue.channel_id}`, guild_id: guildId }, { value: newQueueMsg.id })
                .catch(async (error) => {
              ...
              console.warn(`Queue channel ${queue.channel_id} not found for guild ${guildId}`);
            }
          }
          ...
          await interaction.reply({ content: `An error occurred while processing this action: ${error.message}`, flags: [64] });
        }
      }
